...
void draw() {
  ...
  float lightAngle = frameCount * 0.002;
    lightDir.set(avatar.worldLocation(new Vector(0,0,0)).x(),avatar.worldLocation(new Vector(0,0,0)).y(),avatar.worldLocation(new Vector(0,0,0)).z());
...
void updateDefaultShader() {
    ...
    Vector norm = avatar.worldLocation( new Vector(0,0,0));
    float lightNormalX = norm.x() * modelviewInv.m00 + norm.y() * modelviewInv.m10 + norm.z() * modelviewInv.m20;
    float lightNormalY = norm.x() * modelviewInv.m01 + norm.y() * modelviewInv.m11 + norm.z() * modelviewInv.m21;
    float lightNormalZ = norm.x() * modelviewInv.m02 + norm.y() * modelviewInv.m12 + norm.z() * modelviewInv.m22;
